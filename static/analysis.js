...
async function createWeeksCharts() {
    ...
    let colors = Array(result.spent.length).fill("rgba(143, 221, 255, 0.59)");

    let labels = result.dates;
    let values = result.spent;
    let budget = result.budget;

    spendChartWks = await createSpendingCharts(chartCanvas, colors, labels, values, budget);
    ...
    if (categories.length > 0) {
        ...
        chartCanvas2.parentElement.querySelector("p").hidden = false;
        chartCanvas3.parentElement.querySelector("p").hidden = false;
    }
}
...
async function createMonthsCharts() {
    ...
    spendChartMths = await createSpendingCharts(chartCanvas, colors, labels, values, budget);
...
tabBtns.forEach(button => {
    button.addEventListener("click", () => {
        ...
        setTimeout(() => {
            if (tabName === 'sixmonths') {
                spendChartMths?.resize();
                CatChartMths1?.resize();
                CatChartMths2?.resize();
            } else {
                spendChartWks?.resize();
                CatChartWks1?.resize();
                CatChartWks2?.resize();
            }
        }, 100);
